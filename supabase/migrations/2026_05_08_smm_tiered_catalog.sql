...
-- 2. Insert Starter and Pro tiers.
...
--    own row even though the names repeat across products.
insert into subscription_plans (product_id, name, billing_cycle, duration_months, discount_percent, active, display_order)
select id, 'Monthly',          'monthly', 1,  0,  true, 1 from products where name = 'Social Media — Starter'
union all
select id, 'Annual (15% off)', 'annual',  12, 15, true, 2 from products where name = 'Social Media — Starter'
union all
select id, 'Monthly',          'monthly', 1,  0,  true, 1 from products where name = 'Social Media — Growth'
union all
select id, 'Annual (15% off)', 'annual',  12, 15, true, 2 from products where name = 'Social Media — Growth'
union all
select id, 'Monthly',          'monthly', 1,  0,  true, 1 from products where name = 'Social Media — Pro'
union all
select id, 'Annual (15% off)', 'annual',  12, 15, true, 2 from products where name = 'Social Media — Pro';
