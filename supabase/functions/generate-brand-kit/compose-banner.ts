...
function layoutFor(assetId: ImageAssetId, W: number, H: number): BannerLayout {
  switch (assetId) {
    case 'banner_yt': // 2560×1440, content must stay in the centered 1546×423 TV-safe strip
      return { mode: 'horizontal', box: { x: 600, y: 545, w: 1360, h: 350 }, logoH: 300, taglineSize: 74, ctaSize: 52, withCopy: true }
    case 'banner_x': // 1500×500, avoid bottom-left profile-pic overlap
      return { mode: 'horizontal', box: { x: 470, y: 120, w: 980, h: 260 }, logoH: 210, taglineSize: 52, ctaSize: 38, withCopy: true }
    case 'banner_linkedin_cover': // 1128×191, very short
      return { mode: 'horizontal', box: { x: 70, y: 28, w: 1000, h: 135 }, logoH: 120, taglineSize: 34, ctaSize: 26, withCopy: true }
    case 'banner_fb': // 820×312
      return { mode: 'horizontal', box: { x: 55, y: 66, w: 710, h: 180 }, logoH: 150, taglineSize: 40, ctaSize: 30, withCopy: true }
    case 'banner_ig': // 1080×1920 vertical story
      return { mode: 'vertical', box: { x: 130, y: 360, w: 820, h: 900 }, logoH: 420, taglineSize: 76, ctaSize: 54, withCopy: true }
...
function makeScrim(w: number, h: number, peakAlpha = 150): Image {
  const G = 64
  const grad = new Image(G, G)
  // feather fraction at each edge; plateau in the middle stays at full alpha
...
export async function composeBanner(args: ComposeArgs): Promise<Uint8Array> {
  ...
  if (layout.mode === 'logo-only') {
    // Center the logo over a soft scrim; no text.
    bg.composite(makeScrim(box.w, box.h, 130), box.x, box.y)
    const lg = resizeToHeight(logoImg, layout.logoH)
    bg.composite(lg, Math.round(box.x + (box.w - lg.width) / 2), Math.round(box.y + (box.h - lg.height) / 2))
    return new Uint8Array(await bg.encode())
  }

  // Soft scrim behind the whole content box.
  bg.composite(makeScrim(box.w, box.h, 155), box.x, box.y)
  ...
  if (layout.mode === 'horizontal') {
    ...
    let cy = box.y
    bg.composite(lg, Math.round(cxCenter - lg.width / 2), cy)
